...
> **다음 단계**: Phase 4-2 (표정/감정 변화 도구) 또는 통합 참조 이미지 생성 시스템
> **주의**: Phase 4-1 (포즈 가이드) 기능은 스타일 일관성 저해 문제로 제거됨 (2026-01-07)
...
**제거 완료** (2026-01-07) - Phase 4-1 기능 완전 제거

**제거 이유**:
- 포즈 가이드 이미지(특히 무채색 스케치)를 첫 번째 참조 이미지로 추가하면, 기존 참조 이미지의 색상과 스타일을 무시하는 문제 발생
- 생성된 이미지가 포즈 가이드의 시각적 특성(무채색, 스케치 스타일)을 따라가며 원본 캐릭터의 외형을 제대로 유지하지 못함
- 프롬프트 엔지니어링으로는 이 문제를 해결할 수 없었음 (포즈 방향성만 참고하라는 지시를 무시)
- 오히려 스타일 일관성을 해치는 결과를 초래하여 기능 제거 결정

**제거된 항목**:
- ImageGeneratorPanel.tsx의 포즈 이미지 업로드 UI (드래그 앤 드롭, 미리보기, 삭제 버튼)
- useGeminiImageGenerator.ts의 포즈 가이드 관련 프롬프트 및 로직
- GenerationHistoryEntry의 poseImageBase64 필드
- Tauri 드래그 앤 드롭 리스너 (포즈 이미지용)

**대안**:
- 텍스트 프롬프트로 포즈 설명을 상세히 입력하는 방식으로 회귀
- 참조 이미지는 캐릭터 외형만 표시하고, 포즈는 프롬프트로만 지시
- 이 방법이 스타일 일관성 유지에 더 효과적임
...
### Phase 4-1 포즈 가이드 기능 제거
- **제거 이유**: 포즈 가이드 이미지(특히 무채색 스케치)를 첨부하면 참조 이미지의 색상과 스타일을 무시하는 심각한 문제 발생
- **제거된 항목**:
  - 포즈 이미지 업로드 UI (드래그 앤 드롭, 미리보기, 삭제 버튼)
  - Tauri 네이티브 드래그 앤 드롭 리스너
  - 포즈 가이드 관련 프롬프트 엔지니어링
  - GenerationHistoryEntry의 poseImageBase64 필드
- **대안**: 텍스트 프롬프트로 포즈를 상세히 설명하는 방식이 스타일 일관성 유지에 더 효과적
...
**Phase 3 완료**, **Phase 4-1 제거**로 이제 다음 기능이 작동합니다:
- ✅ 프롬프트 믹서 (3-1)
...
- ✅ 고급 설정 패널 (3-3)
  ...
  - 생성 히스토리 관리 (핀 기능 포함)
- ❌ 포즈 가이드 (4-1) - **제거됨**
  - 포즈 가이드 이미지 첨부 시 스타일 일관성 저해 문제로 제거 (2026-01-07)
  - 텍스트 프롬프트로 포즈 지시하는 방식으로 회귀

프로젝트는 최종 목표인 **"여러 캐릭터 세션을 통합하여 하나의 완성된 일러스트 생성"**을 향해 진행 중입니다. 포즈 가이드 기능은 스타일 일관성을 저해하여 제거되었으며, 텍스트 기반 포즈 지시가 더 안정적인 것으로 확인되었습니다.

---

**문서 버전**: 4.5
**작성일**: 2026-01-07
**주요 업데이트**: Phase 4-1 (포즈 가이드) 기능 제거 - 스타일 일관성 저해 문제로 완전 제거
**다음 업데이트 예정**: Phase 4-2 (표정/감정 변화 도구) 또는 통합 참조 이미지 생성 시스템 구현 시
