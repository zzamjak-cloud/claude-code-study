...
**중요 출력 규칙**:
1. 분석 보고서는 반드시 <markdown_content>와 </markdown_content> 태그 사이에 작성해야 합니다.
2. 사용자와의 대화는 태그 밖에 작성하세요.
3. 마크다운 콘텐츠 첫 줄은 반드시 HTML 주석으로 게임명을 포함해야 합니다:
   <!-- ANALYSIS_TITLE: {게임명} 게임 분석 보고서 -->
   (이 주석은 세션 제목으로만 사용되며 문서에는 표시되지 않습니다)
4. 실제 문서 내용은 h1부터 시작합니다 (타이틀 없이 바로 본문 시작)
