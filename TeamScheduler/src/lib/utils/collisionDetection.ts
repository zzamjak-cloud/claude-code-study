...
export const hasCollision = (
  ...
  return existingSchedules
    .filter(s =>
      s.memberId === newSchedule.memberId &&
      s.id !== newSchedule.id &&
      (s.rowIndex || 0) === (newSchedule.rowIndex || 0)  // 같은 행에서만 검사
    )
    .some(existing => {
...
export const getCollidingSchedules = (
  ...
  return allSchedules.filter(s => {
    ...
    if ((s.rowIndex || 0) !== (schedule.rowIndex || 0)) return false  // 같은 행에서만 검사
